...
class GamepassCalculatorModal(Modal, title="🍣 คำนวณเกมพาส"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            embed = discord.Embed(
                title=f"🎮 Gamepass {format_number(robux)} :robux: = {format_number(price_int)} บาท (1 บาท = {gamepass_rate} :robux:)",
                color=0xFFA500
            )
...
class GamepassBahtCalculatorModal(Modal, title="🍣 คำนวณเงินบาท (เกมพาส)"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            # Create embed
            embed = discord.Embed(
                title=f"🎮 {format_number(int(baht))} บาท = {format_number(robux)} :robux: (1 บาท = {gamepass_rate} :robux:)",
                color=0xFFA500
            )
            embed.set_footer(text="Sushi Shop 🍣")
...
class GroupCalculatorModal(Modal, title="🍣 คำนวณโรกลุ่ม"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            # Create embed
            embed = discord.Embed(
                title=f"👥 Group {format_number(robux)} :robux: = {format_number(int(price))} บาท ({rate_text})",
                color=0xFFA500
            )
            embed.set_footer(text="Sushi Shop 🍣")
...
class GroupBahtCalculatorModal(Modal, title="🍣 คำนวณเงินบาท (โรกลุ่ม)"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            # Create embed
            embed = discord.Embed(
                title=f"👥 {format_number(int(baht))} บาท = {format_number(robux)} :robux: ({rate_text})",
                color=0xFFA500
            )
            embed.set_footer(text="Sushi Shop 🍣")
...
async def tax(ctx, *, expr):
    try:
        ...
        if re.match(r"^\d+$", expr):
            number = int(expr)
            await ctx.send(f"💰 {format_number(number)} :robux:ที่ได้หลังหัก 30% = **{format_number(int(number * 0.7))} :robux:**")
        elif m := re.match(r"^(\d+)-(\d+)%$", expr):
            number = int(m[1])
            percent = int(m[2])
            await ctx.send(f"💰 {format_number(number)} :robux:ที่ได้หลังหัก {percent}% = **{format_number(int(number * (1 - percent/100)))} :robux:**")
        else:
            await ctx.send(
