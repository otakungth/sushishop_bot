...
class GamepassCalculatorModal(Modal, title="🍣 คำนวณเกมพาส"):
    robux_amount = TextInput(
        label="จำนวนโรบัค",
        placeholder="พิมพ์เฉพาะตัวเลขหรือสมการ เช่น 1000 หรือ 500+500 หรือ 1000*2",
        required=True,
        max_length=50
    )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            embed.add_field(name="📝 สูตรที่คำนวณ", value=f"`{self.robux_amount.value}`", inline=False)
...
class GamepassBahtCalculatorModal(Modal, title="🍣 คำนวณเงินบาท (เกมพาส)"):
    baht_amount = TextInput(
        label="จำนวนเงิน (บาท)",
        placeholder="พิมพ์เฉพาะตัวเลขหรือสมการ เช่น 500 หรือ 100+200 หรือ 1000/2",
        required=True,
        max_length=50
    )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            embed.add_field(name="📝 สูตรที่คำนวณ", value=f"`{self.baht_amount.value}`", inline=False)
            embed.set_footer(text="Sushi Shop 🍣")
