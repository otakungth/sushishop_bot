...
anonymous_mode = {}  # Dictionary to store anonymous mode per channel: {channel_id: bool}
...
async def update_main_channel():
    try:
        ...
        async def rng_cb(i):
            # Send ephemeral message instead of editing main embed
            await show_rng_main_menu(i, i.user, ephemeral=True)
...
async def handle_open_ticket(interaction, category_name, stock_type):
    ...
    try:
        ...
        # Initialize anonymous mode for this channel as False (show name by default)
        anonymous_mode[str(channel.id)] = False
    ...
    async def on_submit(self, i):
        ...
        try:
            ...
            if anonymous_option == "ปิด":
                ...
                save_json(ticket_customer_data_file, ticket_customer_data)
                anonymous_mode[str(i.channel.id)] = True
            else:
                anonymous_mode[str(i.channel.id)] = False
...
async def anonymous_cmd(ctx):
    """เปิดโหมดไม่ระบุตัวตน (ซ่อนชื่อผู้ซื้อในใบเสร็จ)"""
    if not ctx.channel.name.startswith("ticket-"):
        await ctx.send("❌ คำสั่งนี้ใช้ได้เฉพาะในตั๋วเท่านั้น", delete_after=5)
        return
    
    channel_id = str(ctx.channel.id)
    anonymous_mode[channel_id] = True
    
    # Update ticket customer data to anonymous
    ticket_customer_data[channel_id] = "ไม่ระบุตัวตน"
    save_json(ticket_customer_data_file, ticket_customer_data)
    
    embed = discord.Embed(
        title="✅ ปิดชื่อผู้ซื้อแล้ว",
        description="ชื่อผู้ซื้อจะถูกซ่อนในใบเสร็จ",
        color=0x00FF00
    )
    await ctx.send(embed=embed)

@bot.command(name="anonymousoff")
@admin_only()
async def anonymous_off_cmd(ctx):
    """ปิดโหมดไม่ระบุตัวตน (แสดงชื่อผู้ซื้อตามปกติ)"""
    if not ctx.channel.name.startswith("ticket-"):
        await ctx.send("❌ คำสั่งนี้ใช้ได้เฉพาะในตั๋วเท่านั้น", delete_after=5)
        return
    
    channel_id = str(ctx.channel.id)
    anonymous_mode[channel_id] = False
    
    # Find the buyer from channel name
    buyer_name = "ไม่ทราบชื่อ"
    try:
    ...
    # Update ticket customer data back to real name
    ticket_customer_data[channel_id] = buyer_name
    save_json(ticket_customer_data_file, ticket_customer_data)
    
    embed = discord.Embed(
        title="✅ เปิดชื่อผู้ซื้อแล้ว",
        description="ชื่อผู้ซื้อจะแสดงตามปกติ",
        color=0x00FF00
    )
    await ctx.send(embed=embed)
        ...
        async def deliver_cb(i):
            ...
            async def confirm_cb(interaction):
                try:
                    ...
                    is_anonymous = anonymous_mode.get(channel_id, False)
                    
                    if self.buyer and not is_anonymous:
                        ticket_customer_data[channel_id] = self.buyer.name
                    else:
                        ticket_customer_data[channel_id] = "ไม่ระบุตัวตน"
                    save_json(ticket_customer_data_file, ticket_customer_data)
                    ...
                    # Show buyer name based on anonymous mode
                    if is_anonymous:
...
# ==================== คำสั่ง ANNOYMOUS (REMOVED CHANNEL NAME CHANGE) ====================
@bot.command()
@admin_only()
...
# ==================== คำสั่ง COIN MANAGEMENT (FIXED - Works with both mention and ID) ====================
async def get_user_id_from_input(user_input: str) -> Optional[str]:
    """Convert user mention or ID to user ID string"""
    ...
    # Check if it's just an ID (all digits)
...
@bot.command()
@admin_only()
async def delcoin(ctx, user: str = None, amount: str = None):
    """ลบ SushiCoin จากผู้ใช้ !delcoin <user/id> <amount>"""
    if not user or not amount:
        embed = discord.Embed(
            title="❌ การใช้งานไม่ถูกต้อง",
            description="**การใช้งาน:** `!delcoin <user/id> <amount>`\n**ตัวอย่าง:** `!delcoin @user 1000` หรือ `!delcoin 123456789 1000`",
            color=0xFF0000
        )
        await ctx.send(embed=embed)
        return
    ...
    try:
        ...
        user_id = await get_user_id_from_input(user)
        if not user_id:
            await ctx.send(f"❌ ไม่พบผู้ใช้ `{user}` - กรุณาใช้ @mention หรือ ID")
            return
        ...
        # Remove coins
        success = remove_user_balance(user_id, amount_int)
...
@bot.command()
@admin_only()
async def addcoin(ctx, user: str = None, amount: str = None):
    """เพิ่ม SushiCoin ให้ผู้ใช้ !addcoin <user/id> <amount>"""
    if not user or not amount:
        embed = discord.Embed(
            title="❌ การใช้งานไม่ถูกต้อง",
            description="**การใช้งาน:** `!addcoin <user/id> <amount>`\n**ตัวอย่าง:** `!addcoin @user 1000` หรือ `!addcoin 123456789 1000`",
            color=0xFF0000
        )
        await ctx.send(embed=embed)
        return
    ...
    try:
        ...
        # Add coins
        new_balance = add_user_balance(user_id, amount_int)
...
# ==================== คำสั่ง GIVE ITEM (FIXED - Works with both mention and ID) ====================
@bot.command()
@admin_only()
async def give(ctx, item_id: str = None, user: str = None, amount: str = None):
    """ให้ไอเทมแก่ผู้ใช้ !give <item_id> <user/id> <amount>"""
    if not item_id or not user or not amount:
        embed = discord.Embed(
            title="❌ การใช้งานไม่ถูกต้อง",
            description="**การใช้งาน:** `!give <item_id> <user/id> <amount>`\n"
                       "**ตัวอย่าง:** `!give myt_1 @user 50` หรือ `!give myt_1 123456789 50`\n\n"
                       "**รายการ item_id:**\n"
    ...
    try:
        ...
        # เพิ่มไอเทมให้ผู้ใช้
        item = ITEMS[item_id]
        add_item_to_inventory(user_id, item_id, amount_int)
...
async def givelist(ctx):
    ...
    embed = discord.Embed(
        ...
        description="ใช้คำสั่ง `!give <item_id> <user/id> <amount>`",
        color=0x00AAFF
    )
    ...
    embed.set_footer(text="ตัวอย่าง: !give myt_1 @user 50")
...
# ==================== RNG MAIN MENU HELPER ====================
async def show_rng_main_menu(interaction_or_message, user: discord.User, ephemeral: bool = False):
    """Show RNG main menu with current balance - reusable function"""
    user_id = str(user.id)
    balance = get_user_balance(user_id)
    ...
    view = RNGMainView(user)
    
    # Check if this is an interaction or message
    if hasattr(interaction_or_message, 'response'):
        if ephemeral:
            await interaction_or_message.response.send_message(embed=embed, view=view, ephemeral=True)
        else:
            await interaction_or_message.response.edit_message(embed=embed, view=view)
    else:
        await interaction_or_message.edit(embed=embed, view=view)
...
async def rng_slash(interaction: discord.Interaction):
    await show_rng_main_menu(interaction, interaction.user, ephemeral=True)
    ...
    async def inventory_button(self, interaction: discord.Interaction, button: Button):
        ...
        if not inventory:
            ...
            async def back_cb(back_interaction):
                ...
                await show_rng_main_menu(back_interaction, self.user)
...
class RollAgainView(View):
    ...
    async def back_button(self, interaction: discord.Interaction, button: Button):
        ...
        await show_rng_main_menu(interaction, self.user)

# ==================== PAWN SHOP SYSTEM (FIXED SELLING WITH ±20% OFFER AND ALL BUTTONS) ====================
CUSTOMER_NAMES = [
...
class PawnShopDealView(View):
                ...
                if success:
                    ...
                    async def sell_again_callback(cont_interaction):
                        ...
                        if not inventory:
                            embed = discord.Embed(
                                title="🏪 ขายไอเทม",
                                description=f"คุณไม่มีไอเทมเหลือให้ขายแล้ว! ไปสุ่มก่อนนะ 🎲\n\n💰 ยอดเงินปัจจุบัน: **{format_number(get_user_balance(user_id))}** 🪙",
                                color=0x808080
                            )
                            ...
                            await cont_interaction.response.edit_message(embed=embed, view=no_items_view)
                            return
                        ...
                        selection_view = View(timeout=60)
                        pawn_main_view.selected_items = []
                        select = MultiItemSelect(self.user, items_list, pawn_main_view)
                        selection_view.add_item(select)
                        
                        # Create sell button
                        sell_btn = Button(label="ขาย", style=discord.ButtonStyle.success, emoji="✅", row=1)
                        
                        async def sell_callback(sell_interaction):
                            if sell_interaction.user != self.user:
                                await sell_interaction.response.send_message("❌ ไม่ใช่เกมของคุณ!", ephemeral=True)
                                return
                            
                            if not pawn_main_view.selected_items:
                                await sell_interaction.response.send_message("❌ กรุณาเลือกไอเทมก่อน", ephemeral=True)
                                return
                            
                            user_id = str(sell_interaction.user.id)
                            selected_items = pawn_main_view.selected_items
                        ...
                        await cont_interaction.response.edit_message(embed=embed, view=selection_view)
                    ...
                    async def back_to_main_callback(cont_interaction):
                        ...
                        await show_rng_main_menu(cont_interaction, self.user)
    ...
    @discord.ui.button(label="👤 คนถัดไป", style=discord.ButtonStyle.secondary, emoji="👤", row=2)
    async def next_person_button(self, interaction: discord.Interaction, button: Button):
        """แสดงลูกค้าคนถัดไปสำหรับไอเทมเดิม"""
        if interaction.user != self.user:
            await interaction.response.send_message("❌ ไม่ใช่เกมของคุณ!", ephemeral=True)
            return
        await self.show_new_customer_for_same_items(interaction)
    
    @discord.ui.button(label="🔙 กลับ", style=discord.ButtonStyle.secondary, emoji="🔙", row=2)
    async def return_button(self, interaction: discord.Interaction, button: Button):
        """กลับไปหน้าร้านค้าหลัก"""
        if interaction.user != self.user:
            await interaction.response.send_message("❌ ไม่ใช่เกมของคุณ!", ephemeral=True)
            return
        ...
        await interaction.response.edit_message(embed=embed, view=PawnShopMainView(self.user))

# ==================== LEADERBOARD SYSTEM (FIXED USERNAME DISPLAY) ====================
async def show_leaderboard(interaction: discord.Interaction):
    ...
    for idx, (user_id, money) in enumerate(top_5):
        try:
            ...
            # Display as username without mention formatting
            username = user.display_name
        except:
            # If can't fetch user, try to get from cache or use ID without mention format
            try:
                user = interaction.guild.get_member(int(user_id))
                if user:
                    username = user.display_name
                else:
                    # Just use the username without mention tags
                    username = f"User-{user_id[-4:]}"  # Show last 4 digits of ID
            except:
    ...
    async def back_callback(back_interaction):
        ...
        await show_rng_main_menu(back_interaction, interaction.user)
...
async def leaderboard_slash(interaction: discord.Interaction):
    ...
    for idx, (user_id, money) in enumerate(top_5):
        try:
            ...
            # If can't fetch user, try to get from cache
            try:
                user = interaction.guild.get_member(int(user_id))
                if user:
                    username = user.display_name
