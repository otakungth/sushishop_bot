...
class MyBot(commands.Bot):
    def __init__(self):
        ...
        # Rate limiters
        self.api_rate_limiter = RateLimiter(max_calls=1, period=1.0)
        self.react_rate_limiter = RateLimiter(max_calls=1, period=0.5)
        self.channel_edit_rate_limiter = RateLimiter(max_calls=1, period=5.0)
...
async def handle_ticket_after_ty(channel, user, robux_amount=None, customer_name=None):
    ...
    try:
        ...
        try:
            ...
            bot.loop.create_task(move_to_transcript_after_delay(channel, user, robux_amount, customer_name, 600))
...
async def update_channel_name():
    ...
    try:
        ...
        if current_time - bot.last_update_time < 60:  # ลดเหลือ 1 นาที
...
# =======================================================================================
# ✅ ฟังก์ชันจัดการการเปิดตั๋ว (FIXED - Added missing welcome message)
# =======================================================================================
async def handle_open_ticket(interaction, category_name, modal_class, stock_type):
    ...
    try:
        ...
        # ===== WELCOME MESSAGE WITH FORM BUTTONS =====
        ...
        welcome_embed.set_footer(text="Sushi Shop • กดปุ่มด้านล่างเพื่อกรอกแบบฟอร์ม")
...
# =======================================================================================
# ✅ Main Shop View (FIXED - Dynamic button updates)
# =======================================================================================
...
# =======================================================================================
# ✅ ฟังก์ชันอัพเดท main channel ด้วย View ใหม่ (FIXED)
# =======================================================================================

async def update_main_channel_with_new_view():
    ...
    try:
        ...
        # Create fresh view with current status
        new_view = MainShopView()
        ...
        if target_message:
            try:
                await bot.channel_edit_rate_limiter.acquire()
                await target_message.edit(embed=embed, view=new_view)
                print("✅ Updated main channel with fresh view")
            except Exception as e:
                print(f"❌ Could not edit message, sending new one: {e}")
                await channel.send(embed=embed, view=new_view)
        else:
            await channel.send(embed=embed, view=new_view)
            print("✅ Sent new main channel message with fresh view")
        
    except Exception as e:
        print(f"❌ Error updating main channel with new view: {e}")
...
# =======================================================================================
# ✅ คำสั่ง !stock - FIXED with immediate view update
# =======================================================================================
    ...
    if stock_type is None:
        ...
        if amount is None:
            ...
            try:
                ...
                # Force immediate update of main channel
                await update_main_channel_with_new_view()
...
# =======================================================================================
# ✅ คำสั่ง !sushi (ยังคงไว้เพื่อความเข้ากันได้)
# =======================================================================================

@bot.command()
@admin_only()
async def sushi(ctx):
    """สลับสถานะร้าน (เปิด/ปิด)"""
    global shop_open
...
async def qr(ctx):
    ...
    qr_url = "https://media.discordapp.net/attachments/1361004239043821610/1473323355791949948/Sushi_SCB.png?ex=6995cacd&is=6994794d&hm=54c27e2f93d1db856673d38ec5c2aaada236ab682cd4a4184f6fa76721e27b09&=&format=webp&quality=lossless&width=1161&height=1107"
...
# =======================================================================================
# ✅ เริ่มต้นบอท WITH RATE LIMIT PROTECTION
# =======================================================================================
