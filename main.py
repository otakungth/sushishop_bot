...
def add_item_to_inventory(user_id: str, item_id: str, amount: int = 1):
    ...
    print(f"✅ เพิ่ม {item_id} ให้ {user_id}")  # debug
...
@bot.command(name="pawnshop", aliases=["pawn", "shop"])
async def pawnshop_prefix(ctx):
    """เปิด Pawn Shop (ใช้ !pawnshop, !pawn, !shop)"""
    user_id = str(ctx.author.id)
    inventory = get_user_inventory(user_id)
    ...
    embed.add_field(
        name="💰 วิธีใช้",
        value="พิมพ์ `!pawn <เลขที่>` เพื่อเลือกไอเทม\nตัวอย่าง: `!pawn 1`",
        inline=False
    )
    embed.add_field(
        name="📊 ระบบราคา",
...
async def pawn_select_prefix(ctx, number: int):
    """เลือกไอเทมใน Pawn Shop ตามเลขที่ (ใช้ !pawn <เลข>)"""
    user_id = str(ctx.author.id)
    inventory = get_user_inventory(user_id)
    ...
    embed.add_field(
        name="📝 วิธีต่อรอง",
        value="พิมพ์ `!accept` เพื่อตกลง\nพิมพ์ `!reject` เพื่อปฏิเสธ\nพิมพ์ `!increase` เพื่อขอเพิ่ม 5%\nพิมพ์ `!decrease` เพื่อขอลด 5%",
        inline=False
    )
...
async def ms_prefix(ctx, size: str = "medium"):
    ...
    embed.add_field(
        name="🎮 วิธีเล่น",
        value=(
            "ใช้เลขพิกัด: `!open <แถว> <คอลัมน์>`\n"
            "ตัวอย่าง: `!open 3 4` (เปิดแถว3 คอลัมน์4)\n"
            "ปักธง: `!flag <แถว> <คอลัมน์>`\n"
            "ดูบอร์ด: `!board`"
        ),
        inline=False
    )
...
async def ms_open_prefix(ctx, row: int, col: int):
    """เปิดช่องใน Minesweeper (ใช้ !open <แถว> <คอลัมน์>)"""
...
async def ms_flag_prefix(ctx, row: int, col: int):
    """ปักธงใน Minesweeper (ใช้ !flag <แถว> <คอลัมน์>)"""
    if str(ctx.author.id) not in bot.ms_games:
        await ctx.send("❌ ยังไม่มีเกมที่กำลังเล่นอยู่! ใช้ `!ms` เพื่อเริ่มเกมใหม่")
        return
