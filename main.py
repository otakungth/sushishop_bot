...
def get_thailand_time():
    try:
        return dt.now(pytz.timezone('Asia/Bangkok'))
    except:
        return dt.utcnow() + datetime.timedelta(hours=7)
...
def parse_expression(expr: str) -> int:
    """Parse mathematical expression and return integer result"""
    try:
...
def load_all_data():
    ...
    total_sp = sum(data.get("sp", 0) for data in user_levels.values())
    print(f"   - Total SP: {format_number(total_sp)}")
...
async def add_sp(user_id, amount):
    ...
    old_sp = user_levels[user_id_str].get("sp", 0)
    user_levels[user_id_str]["sp"] = user_levels[user_id_str].get("sp", 0) + amount
    user_levels[user_id_str]["total_robux"] = user_levels[user_id_str].get("total_robux", 0) + amount
...
class LevelCheckView(View):
    ...
    async def check_callback(self, interaction: discord.Interaction):
        ...
        if user_id_str not in user_levels:
            ...
            sp = user_levels[user_id_str].get("sp", 0)
            total_robux = user_levels[user_id_str].get("total_robux", 0)
    ...
    async def rank_callback(self, interaction: discord.Interaction):
        sorted_users = sorted(user_levels.items(), key=lambda x: x[1].get("sp", 0), reverse=True)[:10]
...
class GamepassCalculatorModal(Modal, title="🍣 คำนวณเกมพาส"):
    robux_amount = TextInput(
        label="จำนวนโรบัค (สามารถใช้ + - * / x ÷)",
        placeholder="เช่น 1000 หรือ 100+200+300 หรือ 100x3",
        required=True,
        max_length=50
    )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            robux = parse_expression(self.robux_amount.value)
...
class GamepassBahtCalculatorModal(Modal, title="🍣 คำนวณเงินบาท (เกมพาส)"):
    baht_amount = TextInput(
        label="จำนวนเงิน (บาท) (สามารถใช้ + - * / x ÷)",
        placeholder="เช่น 500 หรือ 100+200+200",
        required=True,
        max_length=50
    )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            baht = parse_expression(self.baht_amount.value)
            robux = int(baht * gamepass_rate)
            
            embed = discord.Embed(
                title=f"🎮 {format_number(baht)} บาท = {format_number(robux)} {ROBUX_EMOJI}",
                description=f"เรท: 1 บาท = {gamepass_rate} {ROBUX_EMOJI}",
                color=0xFFA500
            )
            embed.set_footer(text="Sushi Shop 🍣")
...
class GroupCalculatorModal(Modal, title="🍣 คำนวณโรกลุ่ม"):
    robux_amount = TextInput(
        label="จำนวนโรบัค (สามารถใช้ + - * / x ÷)",
        placeholder="เช่น 1000 หรือ 100+200+300",
        required=True,
        max_length=50
    )
        ...
        async def deliver_cb(i):
            ...
            async def confirm_cb(interaction):
                try:
                    ...
                    receipt_embed.add_field(name="😊 ผู้ซื้อ", value=buyer_display, inline=False)
                    receipt_embed.add_field(name="✨ ประเภทพรีเมียม", value=self.premium_type, inline=True)
                    amount_int = round_price(self.amount) if self.amount > 0 else 0
                    receipt_embed.add_field(name="💰 ราคา", value=f"{format_number(amount_int)} บาท" if self.amount > 0 else "ไม่ระบุ", inline=True)
...
async def help_cmd(ctx):
    ...
    embed = discord.Embed(
        title="🍣 Sushi Shop Bot - คำสั่งทั้งหมด 🍣",
        description="ยินดีต้อนรับสู่ Sushi Shop Bot! นี่คือรายการคำสั่งทั้งหมด",
        color=0xFFA500
    )
    embed.set_thumbnail(url="https://cdn.discordapp.com/attachments/717757556889747657/1403684950770847754/noFilter.png")
    ...
    embed.add_field(
        name="📊 เครื่องคิดเลข",
        value=(
            "`!calculator` - เปิดเครื่องคิดเลข (รองรับ + - * / x ÷)\n"
            "`!gp <จำนวน>` - คำนวณเกมพาส\n"
            "`!g <จำนวน>` - คำนวณโรกลุ่ม\n"
            "`!gpb <จำนวน>` - คำนวณเงินบาทเป็นเกมพาส\n"
            "`!gb <จำนวน>` - คำนวณเงินบาทเป็นโรกลุ่ม\n"
    ...
    embed.add_field(
        name="🏆 ระบบเลเวล",
        value=(
            "`!level` - ดูระดับและอันดับของคุณ\n"
            "`!checklv [@user]` - ดูเลเวลของผู้ใช้\n"
            "`!fixroles` - ซ่อมแซมบทบาททั้งหมด (Admin)\n"
            "`!fixuserrole @user` - ซ่อมแซมบทบาทเฉพาะผู้ใช้ (Admin)"
        ),
        inline=False
    )
    ...
    embed.add_field(
        ...
        value=(
            "`!open` - เปิดร้าน\n"
            "`!close` - ปิดร้าน\n"
            "`!stock [type] [จำนวน]` - ดู/ตั้งค่าสต๊อก\n"
            "`!rate [เรท]` - ดู/ตั้งค่าเรท\n"
            "`!group [on/off]` - เปิด/ปิดบริการโรกลุ่ม\n"
            "`!premium [on/off]` - เปิด/ปิดบริการพรีเมียม\n"
            "`!setsp @user <จำนวน>` - ตั้งค่า SP ให้ผู้ใช้\n"
            "`!delsp @user <จำนวน>` - ลบ SP ผู้ใช้\n"
            "`!savedata` - บันทึกข้อมูลทั้งหมด\n"
            "`!checkdata` - ตรวจสอบข้อมูล\n"
            "`!backupdata` - สร้าง backup ข้อมูล\n"
            "`!checkdir` - ตรวจสอบโฟลเดอร์ข้อมูล\n"
            "`!checkperms` - ตรวจสอบสิทธิ์บอท"
        ),
        inline=False
    )
    ...
    embed.add_field(
        name="🎫 คำสั่งในตั๋ว",
        value=(
            "`!od <จำนวน>` - รับออเดอร์เกมพาส\n"
            "`!odg <จำนวน>` - รับออเดอร์โรกลุ่ม\n"
            "`!odp <จำนวน>` - รับออเดอร์พรีเมียม\n"
            "`!ty` - ส่งของและเพิ่ม SP ให้ลูกค้า\n"
            "`!tkd` - ปิดและลบตั๋ว\n"
            "`!annoymous` - เปิดโหมดไม่ระบุตัวตน\n"
            "`!annoymous_off` - ปิดโหมดไม่ระบุตัวตน"
        ),
        inline=False
    )
    ...
    embed.add_field(
        name="📌 คำสั่งทั่วไป",
        value=(
            "`!link` - ดูลิงก์กลุ่ม Roblox\n"
            "`!qr` - แสดง QR Code สลิป SCB\n"
            "`!qr2` - แสดง QR Code สลิปกรุงศรี\n"
            "`!love` - ส่งใจให้ Sushi Shop\n"
    ...
    embed.add_field(
        name="💰 เรทปัจจุบัน",
        value=(
            f"🎮 เกมพาส: 1 บาท = {gamepass_rate} {ROBUX_EMOJI}\n"
            f"👥 โรกลุ่ม: {group_rate_low} {ROBUX_EMOJI} (ต่ำกว่า 500 บาท)\n"
            f"👥 โรกลุ่ม: {group_rate_high} {ROBUX_EMOJI} (500 บาทขึ้นไป)"
        ),
        inline=False
    )
    
    embed.set_footer(text="Sushi Shop 🍣 | ใช้ !help เพื่อดูคำสั่งทั้งหมด")
...
async def premium(ctx, status=None):
    ...
    if status is None:
        ...
        embed = discord.Embed(title="✨ สถานะบริการพรีเมียม", description=f"**{current_status}**", color=0x00FF00 if premium_ticket_enabled else 0xFF0000)
        await ctx.send(embed=embed)
        
    elif status.lower() in ["on", "enable", "เปิด"]:
        premium_ticket_enabled = True
        save_stock_values()
        embed = discord.Embed(title="✅ เปิดพรีเมียม", description="เปิดตั๋วพรีเมียมแล้ว", color=0x00FF00)
        await ctx.send(embed=embed)
        await update_main_channel()
        ...
        save_stock_values()
        embed = discord.Embed(title="❌ ปิดพรีเมียม", description="ปิดตั๋วพรีเมียมแล้ว", color=0xFF0000)
        await ctx.send(embed=embed)
        await update_main_channel()
    else:
        embed = discord.Embed(title="❌ การใช้งานไม่ถูกต้อง", description="**การใช้งาน:**\n`!premium` - เช็คสถานะ\n`!premium on` - เปิด Premium ticket\n`!premium off` - ปิด Premium ticket", color=0xFF0000)
        await ctx.send(embed=embed)
...
async def gp(ctx, *, expr):
    try:
        ...
        await ctx.send("❌ กรุณากรอกตัวเลขให้ถูกต้อง เช่น 100 หรือ 100+200+300", delete_after=5)
...
async def gpb(ctx, *, expr):
    try:
        baht = parse_expression(expr)
        robux = int(baht * gamepass_rate)
        await ctx.send(f"🎮 {format_number(baht)} บาท = **{format_number(robux)} {ROBUX_EMOJI}** (Gamepass เรท {gamepass_rate})")
    except ValueError as e:
        await ctx.send(f"❌ {e}", delete_after=5)
    except:
        await ctx.send("❌ กรุณากรอกตัวเลขให้ถูกต้อง เช่น 500 หรือ 100+200+200", delete_after=5)
...
async def gb(ctx, *, expr):
    try:
        ...
        robux = int(baht * rate)
        await ctx.send(f"👥 {format_number(baht)} บาท = **{format_number(robux)} {ROBUX_EMOJI}** ({rate_text})")
    except ValueError as e:
        await ctx.send(f"❌ {e}", delete_after=5)
    except:
        await ctx.send("❌ กรุณากรอกตัวเลขให้ถูกต้อง เช่น 500 หรือ 100+200+200", delete_after=5)
...
async def check_lv_cmd(ctx, user: discord.Member = None):
    ...
    sorted_users = sorted(user_levels.items(), key=lambda x: x[1].get("sp", 0), reverse=True)
...
async def del_sp_cmd(ctx, user: discord.Member, amount: int):
    ...
    if success:
        user_id_str = str(user.id)
        new_sp = user_levels[user_id_str].get("sp", 0) if user_id_str in user_levels else 0
        embed = discord.Embed(
            title="✅ ลบ SP สำเร็จ",
