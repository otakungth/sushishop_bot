...
class TicketActionView(View):
    ...
    async def payment_info(self, interaction: discord.Interaction, button: Button):
        try:
            # สร้างข้อความเลขบัญชีที่สามารถกดค้างคัดลอกได้
            bank_accounts = (
                "**🏦 ช่องทางการโอนเงิน**\n\n"
                "```\n"
                "บัญชี 1: 12345\n"
...
class GiveCreditView(discord.ui.View):
    def __init__(self):
        super().__init__(timeout=None)
        # เพิ่มปุ่มลิงก์ไปที่ห้องรีวิวโดยตรง
        self.add_item(
            discord.ui.Button(
                label="ให้เครดิต ⭐", 
                url="https://discordapp.com/channels/1360990259311018077/1363250076549382246", 
                style=discord.ButtonStyle.link,
                emoji="⭐"
            )
        )
    ...
    if ctx.channel.name.startswith("ticket-"):
        ...
        class TempCloseView(View):
            ...
            async def give_credit(self, interaction: discord.Interaction, button: Button):
                """ปุ่มให้เครดิต - ไปที่หน้าห้องรีวิวโดยตรง"""
                ...
                credit_embed = discord.Embed(
                    ...
                    description=(
                        "ขอบคุณที่ให้เครดิต! 🎉\n\n"
                        "กดปุ่มด้านล่างเพื่อไปที่ห้องรีวิวและให้เครดิตกับผู้ส่งสินค้า"
                    ),
