...
SUSHI_HEART_EMOJI_ID = 1410484970291466300
SUSHI_ROBUX_EMOJI_ID = 1486314072701141074
...
def get_sushi_heart_emoji(guild):
    """Get the sushiheart emoji as a string that Discord will render"""
    if guild:
        emoji = discord.utils.get(guild.emojis, id=SUSHI_HEART_EMOJI_ID)
        if emoji:
            return str(emoji)
    # Fallback to heart emoji if custom emoji not found
    return "❤️"

def get_robux_emoji(guild):
    """Get the robux emoji as a string that Discord will render"""
    if guild:
        emoji = discord.utils.get(guild.emojis, id=SUSHI_ROBUX_EMOJI_ID)
        if emoji:
            return str(emoji)
    # Fallback to R$ if custom emoji not found
    return "R$"
...
class LevelCheckView(View):
    ...
    async def check_callback(self, interaction: discord.Interaction):
        ...
        robux_emoji = get_robux_emoji(interaction.guild)
...
class GamepassTicketModal(Modal, title="📋 แบบฟอร์มกดเกมพาส"):
    ...
    async def on_submit(self, i):
        ...
        try:
            ...
            price = robux / rate
            price_int = round_price(price)
            robux_emoji = get_robux_emoji(i.guild)
        ...
        async def deliver_cb(i):
            ...
            async def confirm_cb(interaction):
                try:
                    ...
                    robux_emoji = get_robux_emoji(self.channel.guild) if self.channel else "R$"
...
async def update_main_channel():
    try:
        ...
        robux_emoji = get_robux_emoji(channel.guild)
...
async def handle_open_ticket(interaction, category_name, stock_type):
    ...
    try:
        ...
        # FIXED: Get the emoji object from the guild and use it
        sushi_heart = discord.utils.get(interaction.guild.emojis, id=SUSHI_HEART_EMOJI_ID)
        if sushi_heart:
            welcome_msg = await channel.send(f"# สนใจซื้ออะไรแจ้งแอดมินได้เลยค่ะ {sushi_heart} {admin_mention}")
        else:
            # Fallback to heart emoji if custom emoji not found
            print(f"⚠️ Custom emoji with ID {SUSHI_HEART_EMOJI_ID} not found in guild!")
            welcome_msg = await channel.send(f"# สนใจซื้ออะไรแจ้งแอดมินได้เลยค่ะ ❤️ {admin_mention}")
...
async def robuxtoday_cmd(ctx):
    robux_emoji = get_robux_emoji(ctx.guild)
    embed = discord.Embed(
        title="📊 ยอดขายโรบัค",
...
@bot.command()
async def love(ctx):
    sushi_heart = get_sushi_heart_emoji(ctx.guild)
    await ctx.send(f"# LOVE YOU {sushi_heart}")
...
async def say(ctx, *, message):
    sushi_heart = get_sushi_heart_emoji(ctx.guild)
    await ctx.send(f"# {message.upper()} {sushi_heart}")
