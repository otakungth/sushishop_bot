...
gamepass_rate, group_rate_low, group_rate_high = 6.5, 4, 4.5
shop_open, group_ticket_enabled = True, True
gamepass_stock, group_stock = 50000, 0

# Channel IDs
MAIN_CHANNEL_ID, SALES_LOG_CHANNEL_ID, CREDIT_CHANNEL_ID = 1361044752975532152, 1402993077643120720, 1363250076549382246
DELIVERED_CATEGORY_ID, ARCHIVED_CATEGORY_ID = 1419565515088597083, 1445086228113264650

# Files
user_data_file, ticket_transcripts_file, ticket_counter_file = "user_data.json", "ticket_transcripts.json", "ticket_counter.json"
ticket_robux_data_file, ticket_customer_data_file = "ticket_robux_data.json", "ticket_customer_data.json"

user_data, ticket_transcripts, ticket_robux_data, ticket_customer_data, user_notes, ticket_activity = {}, {}, {}, {}, {}, {}

LEVELS = {
    1: {"exp": 1, "role_id": 1361555369825927249}, 2: {"exp": 5000, "role_id": 1432070662977093703},
    3: {"exp": 10000, "role_id": 1361555364776247297}, 4: {"exp": 20000, "role_id": 1432075600746643537},
    5: {"exp": 50000, "role_id": 1432075369179254804}, 6: {"exp": 100000, "role_id": 1361554929017294949},
    7: {"exp": 250000, "role_id": 1432077732862492722}, 8: {"exp": 500000, "role_id": 1363882685260365894},
...
# ==================== RATE LIMITER ====================
class RateLimiter:
    def __init__(self, max_calls=1, period=1.0): self.max_calls, self.period, self.calls, self._lock = max_calls, period, [], asyncio.Lock()
...
class MyBot(commands.Bot):
    ...
    async def setup_hook(self):
        ...
        user_data, ticket_transcripts, ticket_robux_data, ticket_customer_data = load_json(user_data_file, {}), load_json(ticket_transcripts_file, {}), load_json(ticket_robux_data_file, {}), load_json(ticket_customer_data_file, {})
...
def get_next_ticket_number():
    ...
    if bot.ticket_counter["date"] != current_date: bot.ticket_counter = {"counter": 1, "date": current_date}
    else: bot.ticket_counter["counter"] += 1
    save_json(ticket_counter_file, bot.ticket_counter)
    return bot.ticket_counter["counter"]

def admin_only():
    async def predicate(ctx):
        if ctx.author.guild_permissions.administrator or ctx.guild.get_role(1361016912259055896) in ctx.author.roles: return True
...
async def add_exp(user_id, exp_amount, guild):
    user_id = str(user_id)
    if user_id not in user_data: user_data[user_id] = {"exp": 0, "level": 0}
    user_data[user_id]["exp"] += exp_amount
    old_level, new_level = user_data[user_id]["level"], 0
    for lvl, data in sorted(LEVELS.items(), reverse=True):
        if user_data[user_id]["exp"] >= data["exp"]: new_level = lvl; break
    user_data[user_id]["level"] = new_level
    save_json(user_data_file, user_data)
    if new_level != old_level and (member := guild.get_member(int(user_id))):
        if old_level > 0 and (old_role := guild.get_role(LEVELS[old_level]["role_id"])) and old_role in member.roles: await member.remove_roles(old_role)
...
async def update_main_channel():
    ...
    try:
        ...
        embed.add_field(name=f"🎮 กดเกมพาส | 📊 Stock: {gamepass_stock} {'🟢' if gamepass_stock > 0 else '🔴'}", value=f"```\nเรท: {gamepass_rate} | โรแท้ยัดกลุ่มได้\nเช็คราคา: !gp <จำนวน>\n```", inline=False)
        embed.add_field(name=f"👥 โรบัคกลุ่ม | 📊 Stock: {group_stock} {'🟢' if group_stock > 0 else '🔴'}", value=f"```\nเรท: {group_rate_low} | 500+ บาท เรท {group_rate_high}\n⚠️ต้องเข้ากลุ่ม 15 วันก่อนซื้อ⚠️\n```", inline=False)
        embed.add_field(name="🏪 สถานะร้าน", value=f"```\n{'🟢 เปิด' if shop_open else '🔴 ปิดชั่วคราว'}\n```", inline=False)
        embed.set_footer(text="Sushi Shop • รับกดเกมพาสและอื่น ๆ", icon_url="https://media.discordapp.net/attachments/717757556889747657/1403684950770847754/noFilter.png")
        ...
        if not shop_open:
            ...
            gamepass_btn = Button(label="เปิดตั๋วกดเกมพาส", style=discord.ButtonStyle.success, emoji="🎮", disabled=False, custom_id="gamepass_btn")
        ...
        if not shop_open:
            ...
            group_btn = Button(label="เปิดตั๋ว Group", style=discord.ButtonStyle.success, emoji="👥", disabled=False, custom_id="group_btn")
        ...
        async def gamepass_cb(i): await handle_open_ticket(i, "🍣Sushi Gamepass 🍣", "gamepass")
        async def group_cb(i): await handle_open_ticket(i, "💰Robux Group💰", "group")
        async def notes_cb(i): await i.response.send_modal(PersonalNoteModal())
        async def level_cb(i): await check_user_level(i)
        
        gamepass_btn.callback, group_btn.callback, notes_btn.callback, level_btn.callback = gamepass_cb, group_cb, notes_cb, level_cb
        
        view.add_item(gamepass_btn).add_item(group_btn).add_item(notes_btn).add_item(level_btn)
...
async def handle_open_ticket(interaction, category_name, stock_type):
    ...
    try:
        if (stock_type == "gamepass" and gamepass_stock <= 0) or (stock_type == "group" and group_stock <= 0):
            await interaction.response.send_message(f"❌ สินค้าหมดชั่วคราว", ephemeral=True)
            return
        if not shop_open:
            await interaction.response.send_message(f"❌ ร้านปิดชั่วคราว กรุณารอให้ร้านเปิด", ephemeral=True)
            return
        
        if existing := discord.utils.get(interaction.guild.text_channels, name=f"ticket-{interaction.user.name}-{interaction.user.id}"):
            view = View().add_item(discord.ui.Button(label="📩 ไปที่ตั๋ว", url=f"https://discord.com/channels/{existing.guild.id}/{existing.id}", style=discord.ButtonStyle.link))
            await interaction.response.send_message("📌 คุณมีตั๋วเปิดอยู่แล้ว! กดปุ่มด้านล่างเพื่อไปที่ตั๋ว", view=view, ephemeral=True)
            return
        ...
        if admin_role := interaction.guild.get_role(1361016912259055896):
            overwrites[admin_role] = discord.PermissionOverwrite(read_messages=True, send_messages=True)
        
        if not (category := discord.utils.get(interaction.guild.categories, name=category_name)):
        ...
        ticket_activity[channel.id] = {'last_activity': get_thailand_time(), 'ty_used': False}
        ticket_customer_data[str(channel.id)] = interaction.user.name
        save_json(ticket_customer_data_file, ticket_customer_data)
        
        if stock_type == "gamepass": gamepass_stock -= 1
        else: group_stock -= 1
        
        await update_main_channel()
        
        view = View().add_item(discord.ui.Button(label="📩 ไปที่ตั๋ว", url=f"https://discord.com/channels/{channel.guild.id}/{channel.id}", style=discord.ButtonStyle.link))
        ...
        if stock_type == "gamepass":
            embed.add_field(name="🎮 บริการกดเกมพาส", value=f"📦 Stock คงเหลือ: **{gamepass_stock}**\n💰 เรท: {gamepass_rate}", inline=False)
        else:
            embed.add_field(name="👥 บริการโรบัคกลุ่ม", value=f"📦 Stock คงเหลือ: **{group_stock}**\n💰 เรท: {group_rate_low} - {group_rate_high}", inline=False)
...
async def save_ticket_transcript(channel, action_by=None, robux_amount=None, customer_name=None):
    try:
        ...
        robux_str = robux_amount if robux_amount else ticket_robux_data.get(str(channel.id), "1099")
        customer_str = customer_name if customer_name else ticket_customer_data.get(str(channel.id), channel.name.split('-')[1] if channel.name.startswith("ticket-") else "wforr")
        filename = f"{date_str}{time_str}-{robux_str}-{customer_str}"
        
        ticket_transcripts[str(channel.id)] = {
            "filename": filename, "channel_name": channel.name, "channel_id": channel.id, "ticket_number": ticket_number,
            "date": date_str, "time": time_str, "timestamp": f"{date_str}{time_str}", "robux_amount": str(robux_str),
            "customer_name": customer_str, "category": channel.category.name if channel.category else "ไม่มีหมวดหมู่",
            "created_at": now.isoformat(), "closed_by": str(action_by) if action_by else "ระบบอัตโนมัติ", "messages_count": 0
        }
...
async def handle_ticket_after_ty(channel, user, robux_amount=None, customer_name=None):
    try:
        ...
        await bot.channel_edit_rate_limiter.acquire()
        await channel.edit(category=delivered_category, reason=f"ย้ายไปห้องส่งของแล้วโดย {user.name if user else 'ระบบ'}")
    ...
    try:
        ...
        if archived_category:
            if user:
                ...
                except Exception as e: print(f"⚠️ ไม่สามารถลบสิทธิ์ view ของผู้ซื้อ: {e}")
            ...
            try:
                await channel.send(embed=discord.Embed(title="📁 ตั๋วถูกย้ายไปเก็บถาวรแล้ว", description=f"ตั๋วนี้ถูกย้ายไปเก็บถาวรเรียบร้อยแล้ว\n\n**ชื่อไฟล์ transcript:** `{channel.name}`", color=0x808080))
            except: pass
    except Exception as e: print(f"❌ Error moving to archive: {e}")

# ==================== MODALS ====================
class PersonalNoteModal(Modal, title="📝 จดวันที่เข้ากลุ่ม"):
    note = TextInput(label="จดวันที่เข้ากลุ่มในนี้ ดูจากวันที่ปัจจุบัน", placeholder="เช่น: 20/10 , 20 ตค", style=discord.TextStyle.paragraph, required=False, max_length=1000)
    async def on_submit(self, i):
        if self.note.value.strip():
            user_notes[str(i.user.id)] = {"note": self.note.value, "created_at": datetime.datetime.now().isoformat(), "updated_at": datetime.datetime.now().isoformat()}
            await i.response.send_message(embed=discord.Embed(title="✅ บันทึกโน้ตเรียบร้อย", color=0x00FF00).add_field(name="📝 จดวันที่เข้ากลุ่ม", value=self.note.value), ephemeral=True)
        else:
            if str(i.user.id) in user_notes: del user_notes[str(i.user.id)]; await i.response.send_message("🗑️ ลบโน้ตส่วนตัวเรียบร้อยแล้ว", ephemeral=True)
            else: await i.response.send_message("❌ คุณยังไม่มีโน้ตที่บันทึกไว้", ephemeral=True)

class GamepassTicketModal(Modal, title="📋 แบบฟอร์มกดเกมพาส"):
    map_name = TextInput(label="🗺 ชื่อแมพที่จะกด?", placeholder="พิมพ์ชื่อแมพ เช่น Gift Simulator", required=True)
    gamepass_name = TextInput(label="💸 กดเกมพาสอะไร?", placeholder="พิมพ์ชื่อเกมพาส เช่น x3 Speed 3 ชิ้น", required=True)
    robux_amount = TextInput(label="🎟 ราคาของเกมพาสเท่าไหร่บ้าง?", placeholder="เช่น 300 หรือ 100+100+100 หรือ 100x3", required=True)
    ...
    async def on_submit(self, i):
        try:
            ...
            async def cancel_cb(interaction): await interaction.response.send_message("❌ คำสั่งซื้อถูกยกเลิก"); await interaction.message.delete()
...
class GroupTicketModal(Modal, title="📋 แบบฟอร์มสั่งซื้อ Robux Group"):
    ...
    robux_amount = TextInput(label="💸 ต้องการซื้อกี่โรบัค?", placeholder="กรอกจำนวนโรบัคที่ต้องการ", required=True)
...
class DeliveryView(View):
    def __init__(self, channel, product_type, robux_amount, price, buyer):
        ...
        self.channel, self.product_type, self.robux_amount, self.price, self.buyer, self.delivered = channel, product_type, robux_amount, price, buyer, False
        
        deliver_btn = Button(label="ส่งสินค้าแล้ว ✅", style=discord.ButtonStyle.success, emoji="✅", custom_id=f"deliver_{channel.id}")
        ...
        async def deliver_cb(i):
                ...
                if msg.author == i.user and msg.attachments:
                    for att in msg.attachments:
                        if any(att.filename.lower().endswith(ext) for ext in ['.png', '.jpg', '.jpeg', '.gif']):
                            delivery_image = att.url; break
                    if delivery_image: break
            
            if not delivery_image:
                await i.response.send_message("❌ ผู้ส่งสินค้าต้องแนบไฟล์หลักฐานการส่งสินค้าก่อน !", ephemeral=True)
                return
            ...
            async def confirm_cb(interaction):
                ...
                receipt_color = 0xFFA500 if self.product_type == "Gamepass" else (0x00FFFF if self.product_type == "Group" else 0x00FF00)
                receipt_embed = discord.Embed(title=f"🍣 ใบเสร็จการสั่งซื้อ ({self.product_type}) 🍣", color=receipt_color)
                ...
                if delivery_image: receipt_embed.set_image(url=delivery_image)
                receipt_embed.set_footer(text=f"จัดส่งสินค้าสำเร็จ 🤗 • {get_thailand_time().strftime('%d/%m/%y, %H:%M')}")
                
                if self.buyer:
                    try:
                        dm_embed = discord.Embed(title=f"🧾 ใบเสร็จการซื้อสินค้า ({self.product_type})", description="ขอบคุณที่ใช้บริการ Sushi Shop นะคะ 🍣", color=receipt_color)
                        ...
                        if delivery_image: dm_embed.set_image(url=delivery_image)
                        dm_embed.add_field(name="📝 หมายเหตุ", value="หากมีปัญหากรุณาติดต่อแอดมินในเซิร์ฟเวอร์", inline=False)
                        dm_embed.set_footer(text="Sushi Shop • ขอบคุณที่ไว้วางใจ 💖")
                        await self.buyer.send(embed=dm_embed)
                    except: pass
                
                if log_channel := bot.get_channel(SALES_LOG_CHANNEL_ID):
                ...
                await interaction.response.edit_message(content="✅ บันทึกการส่งสินค้าเรียบร้อยแล้ว", embed=None, view=None)
            
            async def edit_cb(interaction):
                await interaction.response.send_message("📝 กรุณาแนบไฟล์หลักฐานการส่งสินค้าใหม่ แล้วกดปุ่ม 'ส่งสินค้าแล้ว ✅' อีกครั้ง", ephemeral=True)
            
            confirm_btn.callback, edit_btn.callback = confirm_cb, edit_cb
            confirm_view.add_item(confirm_btn).add_item(edit_btn)
            
            embed = discord.Embed(title="📦 ยืนยันการส่งสินค้า", description="ยืนยันหลักฐานการส่งสินค้านี้หรือไม่?", color=0x00FF00).set_image(url=delivery_image)
...
async def stock(ctx, stock_type=None, amount=None):
    ...
    if not stock_type:
        ...
        if amount is None:
            ...
            embed = discord.Embed(title="✅ ตั้งค่า Stock เรียบร้อย", description=f"ตั้งค่า สต๊อกเกมพาส เป็น **{gamepass_stock:,}** เรียบร้อยแล้ว", color=0x00FF00)
            msg = await ctx.send(embed=embed)
            await asyncio.sleep(3)
            await msg.delete()
            await update_main_channel()
        ...
        if amount is None:
            ...
            embed = discord.Embed(title="✅ ตั้งค่า Stock เรียบร้อย", description=f"ตั้งค่า สต๊อกโรบัคกลุ่ม เป็น **{group_stock:,}** เรียบร้อยแล้ว", color=0x00FF00)
            msg = await ctx.send(embed=embed)
            await asyncio.sleep(3)
            await msg.delete()
            await update_main_channel()
...
async def group(ctx, status=None):
    ...
    if status is None:
        ...
        embed = discord.Embed(title="👥 สถานะ Group Ticket", description=f"**{current_status}**", color=0x00FF00 if group_ticket_enabled else 0xFF0000)
        msg = await ctx.send(embed=embed)
        await asyncio.sleep(3)
        await msg.delete()
    elif status.lower() in ["on", "enable", "เปิด"]:
        group_ticket_enabled = True
        embed = discord.Embed(title="✅ เปิดปุ่ม Group Ticket", description="เปิดปุ่ม Group Ticket เรียบร้อยแล้ว", color=0x00FF00)
        msg = await ctx.send(embed=embed)
        await asyncio.sleep(3)
        await msg.delete()
        await update_main_channel()
        ...
        embed = discord.Embed(title="❌ ปิดปุ่ม Group Ticket", description="ปิดปุ่ม Group Ticket เรียบร้อยแล้ว", color=0xFF0000)
        msg = await ctx.send(embed=embed)
        await asyncio.sleep(3)
        await msg.delete()
        await update_main_channel()
...
async def rate(ctx, rate_type=None, low_rate=None, high_rate=None):
    ...
    if rate_type is None:
        ...
        if low_rate is None or high_rate is None:
            ...
            await msg.delete()
            return
        group_rate_low, group_rate_high = float(low_rate), float(high_rate)
        embed = discord.Embed(title="✅ เปลี่ยนเรท Group เรียบร้อย", description=f"ตั้งค่าเรท Group เป็น **{group_rate_low} - {group_rate_high}** เรียบร้อยแล้ว", color=0x00FF00)
        msg = await ctx.send(embed=embed)
        await asyncio.sleep(3)
        await msg.delete()
        await update_main_channel()
        ...
        embed = discord.Embed(title="✅ เปลี่ยนเรท Gamepass เรียบร้อย", description=f"ตั้งค่าเรท Gamepass เป็น **{gamepass_rate}** เรียบร้อยแล้ว", color=0x00FF00)
        msg = await ctx.send(embed=embed)
        await asyncio.sleep(3)
        await msg.delete()
        await update_main_channel()
    ...
    try:
        ...
        if handle_success and buyer:
            try:
                ...
                thank_you_embed.add_field(name="📌 หมายเหตุ", value="หากมีปัญหาหรือข้อสงสัย สามารถติดต่อแอดมินในเซิร์ฟเวอร์ได้เลยค่ะ", inline=False)
...
async def od(ctx, *, expr):
    ...
    try:
        robux = int(eval(expr.replace(",", "").lower().replace("x", "*").replace("÷", "/")))
        price = robux / gamepass_rate
        
        buyer = None
        parts = ctx.channel.name.split('-')
        if len(parts) >= 3:
            try: buyer = ctx.guild.get_member(int(parts[-1]))
            except: pass
        if not buyer:
            async for msg in ctx.channel.history(limit=20):
                if not msg.author.bot and msg.author != ctx.guild.me: buyer = msg.author; break
        
        if buyer: await add_exp(buyer.id, robux, ctx.guild)
        ...
        await update_main_channel()
    except Exception as e: await ctx.send(f"❌ เกิดข้อผิดพลาด: {e}", delete_after=10)
...
async def qr(ctx):
    await ctx.message.delete()
    embed = discord.Embed(title="⚠️โน๊ตใต้สลิประบุชื่อสินค้าที่ซื้อด้วย⚠️ ช่องทางการโอนเงิน", color=0x00CCFF)
    embed.add_field(name="1. ชื่อบัญชี (ธนาคาร SCB)", value="**หจก. วอเตอร์ เทค เซลล์ แอนด์ เซอร์วิส**", inline=False)
    ...
    async def copy_cb(i): await i.response.send_message(f"```120-239181-3 : ธนาคาร SCB```", ephemeral=True)
...
async def gp(ctx, *, expr):
    ...
    except: await ctx.send("❌ กรุณากรอกตัวเลขให้ถูกต้อง", delete_after=5)
...
async def tax(ctx, *, expr):
    try:
        ...
        if re.match(r"^\d+$", expr):
            ...
            number, percent = int(m[1]), int(m[2])
...
async def level(ctx, member=None):
    if isinstance(member, str): member = ctx.guild.get_member_named(member) or ctx.author
    elif member is None: member = ctx.author
    
    user_id = str(member.id)
    if user_id not in user_data: user_data[user_id] = {"exp": 0, "level": 0}
    
    user_exp, user_level = user_data[user_id]["exp"], user_data[user_id]["level"]
    
    embed = discord.Embed(title=f"🍣 ระดับของคุณ {member.display_name}", color=0x00FF99)
    embed.add_field(name="🎮 ระดับปัจจุบัน", value=f"<@&{LEVELS[user_level]['role_id']}>" if user_level > 0 else "Level 0", inline=True)
    embed.add_field(name="⭐ EXP สะสม", value=f"**{user_exp:,} EXP**", inline=True)
    
    if user_level < 9:
        next_exp = LEVELS[user_level + 1]["exp"]
        embed.add_field(name="🎯 ระดับถัดไป", value=f"ต้องการอีก **{next_exp - user_exp:,} EXP** เพื่อยศ <@&{LEVELS[user_level + 1]['role_id']}>", inline=False)
...
@bot.command()
async def love(ctx): await ctx.send("# LOVE YOU<:sushiheart:1410484970291466300>")

@bot.command()
async def say(ctx, *, message): await ctx.send(f"# {message.upper()} <:sushiheart:1410484970291466300>")
...
async def help_command(ctx):
    ...
    embed.description = (
        "**คำสั่ง Text Commands (ใช้ ! ในเซิร์ฟเวอร์):**\n"
...
async def gamepass_cmd(i, amount: str):
    try:
        robux = int(eval(amount.replace(",", "").replace("x", "*").replace("÷", "/")))
        await i.response.send_message(f"🎮 Gamepass {robux:,} Robux = **{robux/gamepass_rate:,.0f} บาท** (เรท {gamepass_rate})")
    except: await i.response.send_message("❌ กรุณากรอกตัวเลขให้ถูกต้อง", ephemeral=True)
...
async def tax_cmd(i, amount: str):
    try:
        ...
        if re.match(r"^\d+$", amount):
            ...
            await i.response.send_message("❌ รูปแบบไม่ถูกต้อง\n\n**การใช้งาน:**\n`/tax 100` - หัก 30% อัตโนมัติ\n`/tax 100-30%` - หัก 30%\n`/tax 100-50%` - หัก 50%")
    except: await i.response.send_message("❌ กรุณากรอกตัวเลขให้ถูกต้อง", ephemeral=True)
...
async def update_presence():
    await bot.change_presence(activity=discord.Activity(type=discord.ActivityType.watching, name=f"ร้าน Sushi Shop | GP: {gamepass_stock} | Group: {group_stock}"))
