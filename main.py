...
async def order(ctx, *, expression: str):
    """สั่งซื้อ Robux Gamepass/Group โดยแสดงแค่จำนวน Robux + ราคาบาท"""
    try:
        expr = expression.replace(",", "").lower().replace("x", "*").replace("÷", "/")

        # ตรวจสอบตัวเลขและเครื่องหมายเท่านั้น
        if not re.match(r"^[\d\s\+\-\*\/\(\)]+$", expr):
            await ctx.send("❌ กรุณาใส่เฉพาะตัวเลข และเครื่องหมาย + - * / ()")
            return

        robux = int(eval(expr))
        price = robux / gamepass_rate  # ใช้เรท Gamepass หรือปรับเป็น group_rate ตามต้องการ
        price_str = f"{price:,.0f} บาท"
        ...
        embed.add_field(name="💰 ราคาที่ต้องจ่าย", value=price_str, inline=True)
        embed.set_footer(text="ทีมงานจะตอบกลับโดยเร็วที่สุดครับ")
