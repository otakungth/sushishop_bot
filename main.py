...
group_rate_low = 4
group_rate_high = 4.5
shop_open = False  # Starting shop closed
group_ticket_enabled = True
gamepass_stock = 20000  # Starting stock 20000
group_stock = 30000  # Starting stock 30000
...
def load_stock_values():
    ...
    if stock_data:
        ...
        group_stock = stock_data.get("group_stock", 30000)
        ...
        shop_open = stock_data.get("shop_open", False)
...
class GamepassCalculatorModal(Modal, title="🍣 คำนวณเกมพาส"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            embed = discord.Embed(
                title=f"🎮 Gamepass {format_number(robux)} {ROBUX_EMOJI} = {format_number(price_int)} บาท (1 บาท = {gamepass_rate:.1f} {ROBUX_EMOJI})",
                color=0xFFA500
            )
...
class GamepassBahtCalculatorModal(Modal, title="🍣 คำนวณเงินบาท (เกมพาส)"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            # Create embed
            embed = discord.Embed(
                title=f"🎮 {format_number(int(baht))} บาท = {format_number(robux)} {ROBUX_EMOJI} (1 บาท = {gamepass_rate:.1f} {ROBUX_EMOJI})",
                color=0xFFA500
            )
            embed.set_footer(text="Sushi Shop 🍣")
...
class GroupCalculatorModal(Modal, title="🍣 คำนวณโรกลุ่ม"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            # Create embed
            embed = discord.Embed(
                title=f"👥 Group {format_number(robux)} {ROBUX_EMOJI} = {format_number(int(price))} บาท ({rate_text})",
                color=0xFFA500
            )
            embed.set_footer(text="Sushi Shop 🍣")
...
class PremiumView(View):
    def __init__(self):
        ...
        # Add button (disabled based on shop_open status)
        ...
        async def premium_cb(i):
            if not shop_open:
                ...
                return
            await handle_open_ticket(i, "🍣Sushi Premium 🍣", "premium")
...
# ==================== ROACC VIEW ====================
class RoaccView(View):
    def __init__(self):
        super().__init__(timeout=None)
        
        # Add button (disabled based on shop_open status)
        roacc_btn = Button(label="ซื้อรหัสโรบัค", style=discord.ButtonStyle.success if shop_open else discord.ButtonStyle.danger, emoji="🎫", disabled=not shop_open)
        
        async def roacc_cb(i):
            if not shop_open:
                await i.response.send_message("❌ ร้านปิดชั่วคราว กรุณารอเปิดให้บริการ", ephemeral=True)
                return
            await handle_open_ticket(i, "🍣Sushi Robux Account 🍣", "roacc")
        
        roacc_btn.callback = roacc_cb
        self.add_item(roacc_btn)
...
class EmbedShopView(View):
    def __init__(self):
        ...
        # Add buttons (disabled based on shop_open status AND stock availability)
        ...
        group_btn = Button(
            label="เติมโรกลุ่ม", 
            style=discord.ButtonStyle.success if shop_open and group_stock > 0 else discord.ButtonStyle.danger, 
            emoji="👥", 
        ...
        async def gamepass_cb(i):
            if not shop_open:
                ...
                return
            if gamepass_stock <= 0:
                await i.response.send_message("❌ โรบัคเกมพาสหมดชั่วคราว", ephemeral=True)
                return
        ...
        async def group_cb(i):
            if not shop_open:
                ...
                return
            if group_stock <= 0:
                await i.response.send_message("❌ โรบัคกลุ่มหมดชั่วคราว", ephemeral=True)
                return
    ...
    async def update_buttons(self):
        """Update button styles based on shop status and stock"""
        self.clear_items()
...
class MyBot(commands.Bot):
    ...
    def load_all_data(self):
        ...
        print(f"   - Shop Status: {'Open' if shop_open else 'Closed'}")
...
async def calculator_cmd(ctx):
    ...
    embed.add_field(
        name="🎮 เกมพาส",
        value=f"เรท {gamepass_rate:.1f}\n1 บาท = {gamepass_rate:.1f} {ROBUX_EMOJI}",
        inline=True
    )
    embed.add_field(
        name="👥 โรกลุ่ม",
...
async def premium_cmd(ctx):
    """แสดง embed รับเติมพรีเมียม"""
    embed = discord.Embed(
        title="🍣 Sushi Shop รับเติมพรีเมียม 🍣",
        description="กดปุ่มด้านล่างเพื่อเปิดตั๋วสำหรับเติมพรีเมียม",
        color=0xFFA500
    )
    ...
    embed.set_footer(
        text=f"Sushi Shop • รับเติมพรีเมียม |: {get_thailand_time().strftime('%d/%m/%y %H:%M')}",
        icon_url="https://media.discordapp.net/attachments/717757556889747657/1403684950770847754/noFilter.png"
    )
    embed.set_thumbnail(url="https://media.discordapp.net/attachments/717757556889747657/1403684950770847754/noFilter.png")
...
@bot.command(name="roacc")
async def roacc_cmd(ctx):
    """แสดง embed รับซื้อรหัสโรบัค"""
    embed = discord.Embed(
        title="🍣 Sushi Shop Robux account 🍣",
...
async def help_command(ctx, command_name: str = None):
    ...
    if command_name:
        ...
        general_commands = (
            ...
            "`!premium` - แสดง embed รับเติมพรีเมียม\n"
            "`!roacc` - แสดง embed รับซื้อรหัสโรบัค\n"
...
async def show_command_help(ctx, command_name: str):
    commands_info = {
        "calculator": {
            ...
            "usage": "!calculator",
            "example": "!calculator",
            "note": "มีปุ่มให้เลือกคำนวณเกมพาสและโรกลุ่ม รวมถึงคำนวณจากเงินบาท"
        },
        "embedshop": {
        ...
        "premium": {
            "description": "แสดง embed รับเติมพรีเมียม พร้อมปุ่มเปิดตั๋ว",
            "usage": "!premium",
            "example": "!premium",
            "note": "แสดงรายละเอียดการเติมพรีเมียม"
        },
        "roacc": {
            "description": "แสดง embed รับซื้อรหัสโรบัค พร้อมปุ่มเปิดตั๋ว",
            "usage": "!roacc",
            "example": "!roacc",
            "note": "แสดงรายละเอียดการซื้อรหัสโรบัค"
        },
        "gp": {
            "description": f"คำนวณราคา Gamepass จากจำนวน {ROBUX_EMOJI}",
            "usage": "!gp <จำนวน>",
            "example": "!gp 1000",
            "note": "แสดงราคาเป็นบาทตามเรทปัจจุบัน"
        },
        "g": {
            "description": f"คำนวณราคา Group จากจำนวน {ROBUX_EMOJI}",
            "usage": "!g <จำนวน>",
            "example": "!g 1000",
            "note": "แสดงราคาเป็นบาทตามเรทปัจจุบัน (500 บาทขึ้นไปได้เรทพิเศษ)"
        },
        "gpb": {
            "description": f"คำนวณจำนวน {ROBUX_EMOJI} Gamepass จากจำนวนบาท",
            "usage": "!gpb <จำนวน>",
            "example": "!gpb 500",
            "note": f"แสดงจำนวน {ROBUX_EMOJI} ที่จะได้รับตามเรทปัจจุบัน"
        },
        "gb": {
            "description": f"คำนวณจำนวน {ROBUX_EMOJI} Group จากจำนวนบาท",
            "usage": "!gb <จำนวน>",
            "example": "!gb 500",
            "note": f"แสดงจำนวน {ROBUX_EMOJI} ที่จะได้รับตามเรทปัจจุบัน"
        },
        "tax": {
            "description": f"คำนวณจำนวน {ROBUX_EMOJI} หลังจากหักภาษี",
...
async def handle_open_ticket(interaction, category_name, stock_type):
    ...
    try:
        if stock_type == "gamepass" and gamepass_stock <= 0:
            await interaction.response.send_message("❌ โรบัคเกมพาสหมดชั่วคราว", ephemeral=True)
            return
        
        if stock_type == "group" and group_stock <= 0:
            await interaction.response.send_message("❌ โรบัคกลุ่มหมดชั่วคราว", ephemeral=True)
            return
        ...
        # Only update stock for gamepass and group
        if stock_type == "gamepass":
            ...
            async with bot.stock_lock:
                group_stock -= 1
        # premium and roacc don't affect stock
        ...
        if stock_type == "gamepass":
            ...
            embed.add_field(
                ...
                value="กรุณาระบุเกมที่ต้องการเติมพรีเมียม\nและจำนวนเดือนที่ต้องการ", 
                inline=False
            )
        elif stock_type == "roacc":
            embed.add_field(
                name="🎫 บริการซื้อรหัสโรบัค", 
                value=f"กรุณาระบุจำนวนรหัสที่ต้องการซื้อ\nและจำนวน {ROBUX_EMOJI} ต่อรหัส", 
                inline=False
            )
        ...
        if stock_type == "gamepass":
            ...
            form_btn = Button(label="📝 กรอกแบบฟอร์มรหัสโรบัค", style=discord.ButtonStyle.primary, emoji="📝")
            
            async def form_callback(i):
                if i.channel.id == channel.id:
                    modal = RoaccTicketModal()
                    await i.response.send_modal(modal)
                else:
                    await i.response.send_message("❌ คุณไม่สามารถใช้ปุ่มนี้ในช่องอื่นได้", ephemeral=True)
...
class PremiumTicketModal(Modal, title="📋 แบบฟอร์มเติมพรีเมียม"):
    game_name = TextInput(
        label="🎮 ชื่อเกมที่ต้องการเติมพรีเมียม", 
        placeholder="พิมพ์ชื่อเกม เช่น Roblox, Mobile Legends", 
        required=True
    )
    months = TextInput(
        label="📅 จำนวนเดือนที่ต้องการ", 
        placeholder="เช่น 1, 3, 6, 12", 
        required=True
    )
    anonymous = TextInput(
        label="🕵️ ต้องการเปิดชื่อผู้ซื้อหรือไม่?", 
        placeholder="พิมพ์ 'yes' หรือ 'no'", 
        required=True,
        max_length=10
    )
    ...
    async def on_submit(self, i):
        try:
            ...
            if anonymous_option not in ["yes", "no"]:
                await i.response.send_message(
                    "❌ กรุณาพิมพ์ 'yes' หรือ 'no' เท่านั้น", 
                    ephemeral=True
                )
                return
            ...
            if anonymous_option == "no":
                ...
                save_json(ticket_customer_data_file, ticket_customer_data)
                display_option = "ไม่ระบุตัวตน"
            else:
                ticket_anonymous_mode[str(i.channel.id)] = False
                display_option = "แสดงชื่อ"
            
            embed = discord.Embed(title="📨 รายละเอียดการสั่งซื้อ", color=0x00FF99)
            embed.add_field(name="🎮 ชื่อเกม", value=self.game_name.value, inline=False)
            embed.add_field(name="📅 จำนวนเดือน", value=self.months.value, inline=True)
...
# ==================== ROACC TICKET MODAL ====================
class RoaccTicketModal(Modal, title="📋 แบบฟอร์มซื้อรหัสโรบัค"):
    code_amount = TextInput(
        label="🎫 จำนวนรหัสที่ต้องการซื้อ", 
        placeholder="เช่น 1, 2, 3", 
        required=True
    )
    robux_per_code = TextInput(
        label=f"💸 จำนวน {ROBUX_EMOJI} ต่อรหัส", 
        placeholder="เช่น 100, 200, 500", 
        required=True
    )
    anonymous = TextInput(
        label="🕵️ ต้องการเปิดชื่อผู้ซื้อหรือไม่?", 
    ...
    async def on_submit(self, i):
        try:
            ...
            try:
                code_amount = int(self.code_amount.value)
                robux_per_code = int(self.robux_per_code.value)
                total_robux = code_amount * robux_per_code
            except ValueError:
            ...
            embed = discord.Embed(title="📨 รายละเอียดการสั่งซื้อ", color=0x00FF99)
            embed.add_field(name="🎫 จำนวนรหัส", value=f"{code_amount} รหัส", inline=True)
            embed.add_field(name=f"💸 {ROBUX_EMOJI} ต่อรหัส", value=f"{format_number(robux_per_code)}", inline=True)
            embed.add_field(name=f"💰 รวม {ROBUX_EMOJI}", value=f"{format_number(total_robux)}", inline=True)
            embed.add_field(name="🕵️ การแสดงชื่อ", value=display_option, inline=True)
            embed.set_footer(text="แอดมินจะตอบกลับเร็วๆนี้")
...
class GamepassTicketModal(Modal, title="📋 แบบฟอร์มกดเกมพาส"):
    ...
    async def on_submit(self, i):
        ...
        try:
            # Check anonymous option - now accepts 'yes' or 'no'
            anonymous_option = self.anonymous.value.strip().lower()
            if anonymous_option not in ["yes", "no"]:
                await i.response.send_message(
                    "❌ กรุณาพิมพ์ 'yes' หรือ 'no' เท่านั้น", 
...
class GroupTicketModal(Modal, title="📋 แบบฟอร์มสั่งซื้อ Robux Group"):
    ...
    robux_amount = TextInput(
        label=f"💸 ต้องการซื้อกี่{ROBUX_EMOJI}?", 
...
async def rate(ctx, rate_type=None, low_rate=None, high_rate=None):
    ...
    if rate_type is None:
        ...
        embed.add_field(name="🎮 Gamepass Rate", value=f"**{gamepass_rate:.1f}**", inline=True)
        ...
        try:
            ...
            embed = discord.Embed(
                ...
                description=f"ตั้งค่าเรทเกมพาสเป็น **{gamepass_rate:.1f}** เรียบร้อยแล้ว", 
                color=0x00FF00
            )
            await ctx.send(embed=embed)
            await update_main_channel()
...
async def ty(ctx):
    ...
    try:
        ...
        async def order_more_cb(interaction):
            ...
            stock_type = "gamepass" if product_type == "Gamepass" else "group"
...
async def gp(ctx, *, expr):
    ...
    try:
        expr_clean = expr.replace(",", "").lower().replace("x", "*").replace("÷", "/")
        robux = int(eval(expr_clean))
        await ctx.send(f"🎮 Gamepass {format_number(robux)} {ROBUX_EMOJI} = **{format_number(int(robux/gamepass_rate))} บาท** (เรท {gamepass_rate:.1f})")
    except:
        await ctx.send("❌ กรุณากรอกตัวเลขให้ถูกต้อง", delete_after=5)
...
async def g(ctx, *, expr):
    ...
    try:
        ...
        await ctx.send(f"👥 Group {format_number(robux)} {ROBUX_EMOJI} = **{format_number(int(price))} บาท** ({rate_text})")
    except Exception as e:
...
async def gpb(ctx, *, expr):
    ...
    try:
        ...
        await ctx.send(f"🎮 {format_number(int(baht))} บาท = **{format_number(int(baht * gamepass_rate))} {ROBUX_EMOJI}** (Gamepass เรท {gamepass_rate:.1f})")
    except:
        await ctx.send("❌ กรุณากรอกตัวเลขให้ถูกต้อง", delete_after=5)
