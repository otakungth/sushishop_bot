...
async def anonymous(ctx):
    ...
    try:
        ...
        # Send confirmation message only - no channel rename
        embed = discord.Embed(
...
async def tkd(ctx):
    ...
    # รูปแบบที่อนุญาต
    valid_formats = [
        channel_name.startswith("ticket-"),  # รูปแบบ ticket-user-userid
    ]
    ...
    if re.match(pattern, channel_name):
        valid_formats.append(True)
    
    if not any(valid_formats):
        await ctx.send("❌ คำสั่งนี้ใช้ได้เฉพาะในช่องตั๋วเท่านั้น", delete_after=5)
        return
