...
async def roacc_cmd(ctx):
    """แสดง Robux account shop"""
    embed = discord.Embed(
        title="🍣 Sushi Shop Robux account 🍣",
        description="รับซื้อรหัสโรบัค พร้อมบริการเติมรหัสโรบัค",
        color=0xFFA500
    )
    embed.add_field(
        name="✨ วิธีการซื้อ",
        value="กดปุ่มด้านล่างเพื่อเปิดตั๋วและแจ้งแอดมิน",
        inline=False
    )
    embed.set_thumbnail(url="https://media.discordapp.net/attachments/717757556889747657/1403684950770847754/noFilter.png")
    embed.set_footer(text="Sushi Shop 🍣")
    
    view = RobuxAccountView()
    await ctx.send(embed=embed, view=view)

# ==================== ROACC VIEW ====================
class RobuxAccountView(View):
    def __init__(self):
        super().__init__(timeout=None)
        
        # Add button (disabled based on shop_open status)
        buy_btn = Button(label="ซื้อรหัสโรบัค", style=discord.ButtonStyle.success if shop_open else discord.ButtonStyle.danger, emoji="🛒", disabled=not shop_open)
    ...
    async def update_button(self):
        """Update button style based on shop status"""
        self.clear_items()
...
async def help_command(ctx, command_name: str = None):
    ...
    if command_name:
        ...
        general_commands = (
            ...
            "`!roacc` - แสดงร้านค้าโรบัคแอคเคาท์\n"
        ...
        if ctx.author.guild_permissions.administrator:
            admin_commands = (
                ...
                "`!stock [gp/group/premium] [จำนวน]` - ดู/ตั้งค่า stock\n"
                "`!group [on/off]` - เปิด/ปิด Group ticket\n"
                "`!premium [on/off]` - เปิด/ปิด Premium ticket\n"
...
async def show_command_help(ctx, command_name: str):
    commands_info = {
        ...
        "roacc": {
            "description": "แสดงร้านค้า Robux Account พร้อมปุ่มซื้อรหัสโรบัค",
            "usage": "!roacc",
            "example": "!roacc",
            "note": "กดปุ่มเพื่อเปิดตั๋วซื้อรหัสโรบัค"
        },
        "gp": {
        ...
        "stock": {
            ...
            "usage": "!stock หรือ !stock gp/group/premium <จำนวน>",
...
class GroupTicketModal(Modal, title="📋 แบบฟอร์มสั่งซื้อ Robux Group"):
    ...
    anonymous = TextInput(
        label="🕵️ ต้องการเปิดชื่อผู้ซื้อหรือไม่?", 
        placeholder="พิมพ์ 'yes' หรือ 'no'",  # Changed to Thai
        required=True,
        max_length=10
    )
    ...
    async def on_submit(self, i):
        ...
        try:
            # Check anonymous option - now accepts Thai 'แสดง' or 'ปิด'
            anonymous_option = self.anonymous.value.strip().lower()
            
            # Update to accept Thai options
            if anonymous_option not in ["แสดง", "ปิด", "yes", "no"]:
                await i.response.send_message(
                    "❌ กรุณาพิมพ์ 'yes' หรือ 'no' เท่านั้น", 
                    ephemeral=True
                )
            ...
            # Convert to Thai display option
            if anonymous_option in ["ปิด", "no"]:
                ticket_anonymous_mode[str(i.channel.id)] = True
                ticket_customer_data[str(i.channel.id)] = "ไม่ระบุตัวตน"
                save_json(ticket_customer_data_file, ticket_customer_data)
                display_option = "ไม่ระบุตัวตน"
        ...
        async def deliver_cb(i):
            ...
            async def confirm_cb(interaction):
                try:
                    ...
                    receipt_embed.add_field(
                        name="💰 ราคา", 
                        value=f"{format_number(int(self.amount))} บาท" if self.amount > 0 else "ไม่ระบุ", 
                        inline=True
                    )
...
async def gp(ctx, *, expr):
    ...
    try:
        expr_clean = expr.replace(",", "").lower().replace("x", "*").replace("÷", "/")
        robux = int(eval(expr_clean))
        await ctx.send(f"🎮 Gamepass {format_number(robux)} {ROBUX_EMOJI} = **{format_number(int(robux/gamepass_rate))} บาท** (เรท {gamepass_rate})")
    except:
        await ctx.send("❌ กรุณากรอกตัวเลขให้ถูกต้อง", delete_after=5)
