...
class DeliveryView(View):
    ...
    try:
        ...
        # ========== แก้ไขตรงนี้: ส่ง DM เฉพาะเมื่อไม่ใช่การสั่งของต่อ ==========
...
async def ty(ctx):
    ...
    try:
        ...
        # ดึงข้อมูลที่เก็บไว้ (ต้องทำก่อนสร้างใบเสร็จ)
        ...
        # ========== สำคัญ: เพิ่ม SP ให้ผู้ซื้อ BEFORE สร้างใบเสร็จ ==========
        if buyer:
            await add_buyer_role(buyer, ctx.guild)
            print(f"✅ Found buyer: {buyer.name} (ID: {buyer.id})")
            
            # เพิ่ม SP ให้กับผู้ซื้อตามจำนวนโรบัค
